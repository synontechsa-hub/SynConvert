...
Commands:
    synconvert scan     --input <dir> [--output <dir>]
    synconvert convert  --input <dir> --output <dir> [--preset <name>] [--encoder <enc>]
                        [--no-review] [--template <fmt>]
    synconvert status   [--queue <file>]
    synconvert presets

Usage examples:
    python -m synconvert scan --input "D:/Anime/Failure Frame"
    python -m synconvert convert --input "D:/Anime/Failure Frame" --output "D:/Mobile/Failure Frame"
    python -m synconvert convert --input "D:/Anime" --output "D:/Mobile" --preset 480p_saver --no-review
"""
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    failed_jobs = [j for j in queue.all_jobs() if j.status.value == "failed"]
    if failed_jobs:
        print("\nFailed jobs:")
