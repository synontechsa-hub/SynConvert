...
class SynLogger:
    ...
    def session_start(self, total_files: int, encoder_label: str):
        print()
        print(_fmt("━" * 60, _BOLD))
        print(_fmt("  SynConvert v1.0.3", _BOLD, _CYAN))
    ...
    def file_success(self, source: str, output: str, preset: str, encoder: str, start_time: float):
        elapsed = time.monotonic() - start_time
        print(f"        {_fmt('✓', _GREEN)} Done in {self._format_duration(elapsed)}")
    ...
    def file_failed(self, source: str, output: str, preset: str, encoder: str, start_time: float, error: str):
        elapsed = time.monotonic() - start_time
        print(f"        {_fmt('✗', _RED)} FAILED: {error}")
        self._records.append(ConversionRecord(
            source=source, output=output, preset=preset,
    ...
    def info(self, msg: str): print(f"  {_fmt('ℹ', _CYAN)} {msg}")
    def warning(self, msg: str): print(f"  {_fmt('⚠', _YELLOW)} {msg}")
    def error(self, msg: str): print(f"  {_fmt('✗', _RED)} {msg}")
