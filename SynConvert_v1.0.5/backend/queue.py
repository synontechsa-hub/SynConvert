...
class JobQueue:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            # FIX #3: Jobs stuck as in_progress mean the app crashed last run.
            # Reset them to pending so they get retried on next run.
            recovered = 0
            for job in self._jobs:
                if job.status == JobStatus.IN_PROGRESS:
                    ...
                    recovered += 1
            if recovered:
                print(f"  ⟳ Recovered {recovered} interrupted job(s) from previous session.")
                self._save()
